...
behData = read.csv("CDVMBG_BRM_Massa_RT.csv")
...
timeLockToRT = function(timeCourseData, behavioralData){
  ...
  for (row in 1:nrow(timeCourseData)){
    ...
    if (RT_asSample < 0){RT_asSample = 1}
...
#   errorsTimeLocked[1:2, c] = quantile(replicate(10000,mean(sample(timeLockedSHAPs[ , c],length(timeLockedSHAPs[ , c]), replace =TRUE), na.rm = TRUE)),c(.975,.025)) #time locked
#   print(c)
# }
#write.csv(errorsTimeLocked,"CDVMBG_Massa_shapErrorBars.csv")
...
errorsTimeLocked = read.csv("CDVMBG_Massa_shapErrorBars.csv")
...
h = ggplot() +
  ...
  coord_cartesian(xlim = c(0,1175), ylim = c(0, 0.2)) +
  theme_classic()
plot(h)
